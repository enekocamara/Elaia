...
public class CustomWorld {
    ...
    public CustomWorld(DimensionType dimension_type, long seed) {
        ...
        Elaia.LOGGER.info("Custom world constructor");
        this.seed = seed;
        this.dimension_type = dimension_type;
        try {
            this.continental_texture = new ContinentalTexture(seed);
        }
        catch(Exception e) {
...
/*
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (openShaderScreen.wasPressed()) {
                if (noiseScreen == null) {
                    noiseScreen = new NoisePreviewScreen(width, height,
                            continental_texture.get_continental_texture().getTextureID(),
                            "continental_noise");
                    LOGGER.info("Creating NoisePreviewScreen");
                }
    ...
    //    return continental_texture.get_continental_texture().getTextureID();
    //}

    public Chunk populateNoise(Chunk chunk){
        ChunkPos chunkPos = chunk.getPos();
        BlockPos.Mutable pos = new BlockPos.Mutable();
        Elaia.LOGGER.info("populate noise: bottomY = " + chunk.getBottomY() + "; dimension_height = " + dimension_type.height() + "; chunk height = " + chunk.getHeight());
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                for (int y = chunk.getBottomY(); y < dimension_type.height(); y++) {
                    ...
                    if (pos.getZ() > 2000 || pos.getZ() < 0 || pos.getX() > 2000 || pos.getX() < 0)
                        continue;
                    if (pos.getY() < 0){
                        Elaia.LOGGER.error("height negative:" + pos.getY());
                    }
