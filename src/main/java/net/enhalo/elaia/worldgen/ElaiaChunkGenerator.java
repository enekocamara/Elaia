...
//import net.enhalo.elaia.TutorialModClient;
...
public class ElaiaChunkGenerator extends ChunkGenerator {
...
""";
    ...
    public ElaiaChunkGenerator(BiomeSource biomeSource, int seaLevel, int worldHeight, Identifier customBlockID) {
        super(biomeSource);
        Elaia.LOGGER.info("chunk generator starting");
    ...
    /*public static final Codec<ElaiaChunkGenerator> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                    BiomeSource.CODEC.fieldOf("biome_source").forGetter(ElaiaChunkGenerator::getBiomeSource),
                    Codec.INT.fieldOf("sea_level").forGetter(ElaiaChunkGenerator::getSeaLevel),
                    Codec.INT.fieldOf("world_height").forGetter(ElaiaChunkGenerator::getWorldHeight),
                    Identifier.CODEC.fieldOf("custom_block").forGetter(ElaiaChunkGenerator::getCustomBlockID)
            ).apply(instance, ElaiaChunkGenerator::new)
    );*/
    public static final MapCodec<ElaiaChunkGenerator> CODEC = RecordCodecBuilder.mapCodec(instance ->
            instance.group(
                    BiomeSource.CODEC.fieldOf("biome_source").forGetter(ElaiaChunkGenerator::getBiomeSource),
                    Codec.INT.fieldOf("sea_level").forGetter(ElaiaChunkGenerator::getSeaLevel),
                    Codec.INT.fieldOf("world_height").forGetter(ElaiaChunkGenerator::getWorldHeight),
    ...
    public CompletableFuture<Chunk> populateNoise(Blender blender, NoiseConfig noiseConfig, StructureAccessor structureAccessor, Chunk chunk) {
        Elaia.LOGGER.info("populate noise called");
        /*
        if (world == null){
            boolean updated = world_generating.compareAndSet(false, true);
            if (updated) {
                world = Elaia.worldManager.world;
            }
        }
        int bottomY = chunk.getBottomY();
        if (bottomY != 0){
            Elaia.LOGGER.error("bottomY is not 0");
